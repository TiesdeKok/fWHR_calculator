...
def log_function(message, name):
    with open(name, 'a') as file:
        file.write(amsterdam.localize(datetime.now()).strftime('%H:%m:%S %d-%m-%y'
                   ) + ' | ' + message + '\n')
...
def get_face_points(points, method='average', top='eyebrow'):
    ...
    if method == 'left':
        coords = (width_left[0], width_right[0], top_left[1],
                  bottom_left[1])
    elif method == 'right':

        coords = (width_left[0], width_right[0], top_right[1],
                  bottom_right[1])
    else:

        top_average = int((top_left[1] + top_right[1]) / 2)
        bottom_average = int((bottom_left[1] + bottom_right[1]) / 2)
        coords = (width_left[0], width_right[0], top_average,
                  bottom_average)
...
def show_box(image, corners):
    ...
    d.line([corners['bottom_left'], corners['top_left']],
           width=line_width)
    d.line([corners['bottom_left'], corners['bottom_right']],
           width=line_width)
    d.line([corners['top_left'], corners['top_right']],
           width=line_width)
    d.line([corners['top_right'], corners['bottom_right']],
           width=line_width)
...
def calculate_fwhr(url, method='average', top='eyelid'):
    log_function(' Start | {}'.format(url), 'custom_log.txt')
    ...
    if good_picture_check(landmarks_as_tuples):
        corners = get_face_points(landmarks_as_tuples, method=method,
                                  top=top)
        ...
        log_function(' End Good | {}'.format(url), 'custom_log.txt')
        return (fwh_ratio, 'data:image/jpeg;base64,'
                + base64.b64encode(bgBase64Data).decode())
    else:
        log_function(' End Bad | {}'.format(url), 'custom_log.txt')
        return 'Not_Suitable'
    ...
    def get(self):
        ...
        if url == None and DEBUG:
            ...
            try:
                ...
                abort(404,
                      message='Calculation failed for unknown reasons.')

            if response == 'Invalid_Filetype':
                abort(404,
                      message='Not a valid filetype, use either jpg or png.'
                      )
            elif response == 'Not_Suitable':

                abort(404,
                      message='Picture not suitable for FWHR calculation.'
                      )
            else:
...
if __name__ == '__main__':
    ...
    log_function(' Restarted Flask Server', 'custom_log.txt')
